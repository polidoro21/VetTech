...
<head>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .input-field { transition: box-shadow .25s ease, border-color .25s ease; }
        .input-field:focus {
            outline: none;
            border-color: #2563EB;
...
<main class="min-h-screen flex">
    ...
    <section class="flex-1 flex items-center justify-center bg-slate-50 px-5 py-10 sm:px-10 relative overflow-hidden">
        ...
        <div class="glass-card w-full max-w-md rounded-3xl shadow-[0_8px_40px_rgba(0,0,0,0.10)] p-8 sm:p-10 animate-fade-up">

            <!-- Logo -->
            <div class="flex flex-col items-center mb-8 animate-fade-up delay-100">
                <div class="w-14 h-14 rounded-2xl bg-gradient-to-br from-brand to-accent flex items-center justify-center shadow-lg mb-4">
            ...
            <!-- Mensagem de sucesso (ex: vindo do cadastro) -->
            @if (session('success'))
                <div class="mb-5 flex items-center gap-2 bg-accent/10 border border-accent/30 text-accent-dark text-sm font-medium px-4 py-3 rounded-xl animate-fade-up">
            ...
            @if ($errors->any() && !$errors->has('email') && !$errors->has('password'))
                <div class="mb-5 flex items-center gap-2 bg-red-50 border border-red-200 text-red-600 text-sm font-medium px-4 py-3 rounded-xl animate-fade-up">
            ...
            <!-- ══ FORMULÁRIO ══ -->
            {{-- CORREÇÃO: action aponta para login.post, método POST --}}
            <form id="loginForm" action="{{ route('login.post') }}" method="POST" class="space-y-5">
                ...
                <div class="animate-fade-up delay-300">
                    ...
                    <div class="relative">
                        ...
                        <input
                            ...
                            required
                            class="input-field w-full pl-10 pr-4 py-3 rounded-xl border {{ $errors->has('email') ? 'border-red-400 bg-red-50' : 'border-slate-200 bg-white' }} text-slate-800 text-sm"
                        />
                    </div>
                    @error('email')
                ...
                <div class="animate-fade-up delay-400">
                    ...
                    <div class="relative">
                        ...
                        <input
                            ...
                            placeholder="••••••••"
                            required
                            class="input-field w-full pl-10 pr-11 py-3 rounded-xl border {{ $errors->has('password') ? 'border-red-400 bg-red-50' : 'border-slate-200 bg-white' }} text-slate-800 text-sm"
                        />
                        <button type="button" id="togglePw" class="absolute inset-y-0 right-3.5 flex items-center text-slate-400 hover:text-slate-600 transition-colors">
                ...
                <div class="flex items-center justify-between animate-fade-up delay-500">
                    <label class="flex items-center gap-2 cursor-pointer select-none">
                        {{-- CORREÇÃO: name="remember" para o controller capturar --}}
                        <input type="checkbox" name="remember" id="remember" class="rounded" />
                        <span class="text-sm text-slate-600">Lembrar de mim</span>
                    </label>
                    <a href="#" class="text-sm font-semibold text-brand hover:text-brand-dark transition-colors hover:underline underline-offset-2">
                        Esqueci minha senha
                    </a>
                </div>
            ...
            <p class="text-center text-sm text-slate-500 mt-7 animate-fade-up delay-700">
                Não possui conta?
                <a href="{{ route('cadastro') }}" class="font-semibold text-brand hover:text-brand-dark transition-colors hover:underline underline-offset-2">
                    Criar conta grátis
                </a>
            </p>
...
<script>
     ...
     * Antes: e.preventDefault() impedia o envio real ao servidor.
     * Agora: apenas exibe o loader e deixa o form submeter normalmente para o Laravel.
     */
    ...
    form.addEventListener('submit', () => {
        /* Feedback visual enquanto a página carrega */
        submitBtn.disabled = true;
        btnText.textContent = 'Entrando…';
        btnIcon.classList.add('hidden');
        btnLoader.classList.remove('hidden');
        /* O formulário segue normalmente para o Laravel */
    });
</script>
